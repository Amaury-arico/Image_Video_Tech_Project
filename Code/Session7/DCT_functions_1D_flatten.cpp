...
float* Dice_to_1D(float* image_dice, size_t height_dim, size_t width_dim, size_t block_size){
    size_t size = height_dim*width_dim;
    float* image_1d = new float [size];
    int block_y = height_dim/block_size;
    int block_x = width_dim/block_size;
    size_t block_elem = block_size*block_size;

    for (int k=0; k < block_y; k++){
        for (int j=0; j < block_x; j++){
            for (int w =0; w<block_size;w++){
                for (int i=0 ; i<block_size ; i++ ){
                    image_1d[i+w*block_size+j*block_elem+(k*block_elem*block_x)] = image_dice[(k*block_size*width_dim)+j*block_size+i+w*width_dim]; // Re build of vector with each group of 8 pixels aligned one after another instead that making a jump of 256 in the memory
                }
            }
        }
    }
...
float* oned_to_Dice(float* image_1d, size_t height_dim, size_t width_dim, size_t block_size){
    size_t size = height_dim*width_dim;
    float* image_dice = new float [size];
    int block_y = height_dim/block_size;
    int block_x = width_dim/block_size;
    size_t block_elem = block_size*block_size;
    ...
    for (int k=0; k < block_y; k++){
        for (int j=0; j < block_x; j++){
            for (int w =0; w<block_size;w++){
                for (int i=0 ; i<block_size ; i++ ){
                    image_dice[(k*block_size*width_dim)+j*block_size+i+w*width_dim] = image_1d[i+w*block_size+j*block_elem+(k*block_elem*block_x)]; // Re build of vector with each group of 8 pixels aligned one after another instead that making a jump of 256 in the memory
                }
            }
        }
    }
...
float* encoder(float * image, float * quant_table, size_t height_dim, size_t width_dim, size_t block_size){
    ...
    float* image_1D = Dice_to_1D(image,height_dim,width_dim,block_size);

    for (int by=0; by < block_y; by++){
        for (int bx=0 ; bx < block_x; bx++){

            for (int k = 0; k < block_elem; k++) {
                image_block[k] = image_1D[(by * block_x + bx) * block_elem + k];
            }
...
float* decoder(float * quantized_image, float * quant_table, size_t height_dim, size_t width_dim, size_t block_size){
    ...
    float* image_1D = Dice_to_1D(quantized_image,height_dim,width_dim,block_size);

    for (int by=0; by < block_y; by++){
        for (int bx=0 ; bx < block_x; bx++){

            for (int k = 0; k < block_elem; k++) {
                extract_quantized[k] = image_1D[(by * block_x + bx) * block_elem + k];
            }
...
float* delta_dc(float* DC_coef, size_t DC_size){

    float* delta = new float[DC_size];
    float prev = 0;
    for (size_t i = 0; i < DC_size; i++) {
        float dc = DC_coef[i];
        delta[i] = dc-prev;
...
float* reconstruct_from_delta(float* delta_dc,size_t vec_size){
    float* reconstruct_DC = new float [vec_size];

    float prev = 0.0f;
    for (size_t i = 0; i < vec_size; i++) {
        prev += delta_dc[i];
        reconstruct_DC[i] = prev;
    }
    return reconstruct_DC;
}
...
std::vector <std::vector<Sequence>> encoder_RLE(float * image, float * quant_table, float *& dc_coef, size_t height_dim, size_t width_dim, size_t block_size){
    ...
    float* image_block = new float [size];
    float* dc_absolute = new float [size/block_elem];
    ...
    for (int by=0; by < block_y; by++){
        for (int bx=0 ; bx < block_x; bx++){
            ...
            dc_absolute[by*block_x+bx] = Quantized_block[0];
    ...
    dc_coef = delta_dc(dc_absolute,size/block_elem);
...
float* decoder_RLE(float * quant_table, std::vector <std::vector<Sequence>> AC, float* dc_coef, size_t height_dim, size_t width_dim, size_t block_size){
    ...
    float* dc_reconstr = reconstruct_from_delta(dc_coef,size/block_elem);

    size_t list_size = AC.size();
    cout << "Size of AC RLE :" << list_size <<endl;
    for (int i = 0; i < list_size ; i++){
        float* reconstruct_block = AC_sequence_decode(dc_reconstr[i], AC[i],block_elem);
        for (int k=0; k < block_elem; k++){
            reconstruct_DCT[i*block_elem+k] = reconstruct_block[k];
        }
        delete [] reconstruct_block;
    }
    ...
    for (int by=0; by < block_y; by++){
        for (int bx=0 ; bx < block_x; bx++){
            ...
            for (int k = 0; k < block_elem; k++) {
                extract_quantized[k] = reconstruct_DCT[(by * block_x + bx) * block_elem + k];
            }
            ...
            float * IDCT_block = IDCT_2D(dequant_block,normal_DCT_base,block_size,block_size);
...
std::vector <Occurence> occurence_calc(std::vector <std::vector<Sequence>> AC ){

    std::vector <Occurence> symb_occurence;

    for (const auto& AC_block : AC){
        for (const auto& seq : AC_block){
            int current_zero = seq.nbre_zero;
            for (auto v : seq.val){
                int current_coeff = static_cast<int> (v);

                bool found = false;
                for (auto& symb : symb_occurence){
                    if (symb.nbre_zero == current_zero  && symb.val == current_coeff){
                        found = true;
                        symb.counter ++;
                        break;
                    }
                ...
                if(!found){
                    symb_occurence.push_back({current_zero,current_coeff,1});
                }
                current_zero = 0;
            }
        }
    ...
    return symb_occurence;
}


std::vector<float> norm_occurence(std::vector<Occurence> hist){

    int max_val = 0;
    std::vector<float> image_counter;

    for (const auto& seq : hist){
        if (seq.counter > max_val){
            max_val = seq.counter;
        }
        image_counter.push_back(static_cast<float>(seq.counter));
    }

    for (const auto& seq : hist){
        if (seq.counter == max_val){
            cout<<"the symbol with max occurence is : {"<< seq.nbre_zero<<","<<seq.val<<"}"<<endl;
        }
    }

    cout << "Maximum occurences is : "<<max_val<<endl;
    for (auto& s : image_counter){
        s /= max_val;
    }
...
float entropy_calc(std::vector<Occurence> hist){

    int tot_symbol = 0;

    for(const auto& seq : hist){
        tot_symbol += seq.counter;
    }

    float H = 0.0f;
    for(const auto& seq : hist){
        float prob = static_cast<float> (seq.counter) / tot_symbol;
        H -= prob*log2(prob);
    }

    cout << "Entropy for image encoding is : "<< H << endl;
    return H;
...
string glob(int n){
    ...
    string suffix = std::bitset<32>(code).to_string().substr(32 - (index + 1));
    return prefix+suffix;
}

int golomb(std::istream& bits){
    ...
    for (int i = 0; i < index; i++){
        bits.get(c);
        code = (code << 1) | (c - '0'); // OR operator ! (c-'0') return a integer from ascii substraction '0' is a char
    }
    return code-1;      // code = n+1 ! -> return code - 1 = n !
}

void write_binary_file(const string& bitstream,const string& filename){
    std::ofstream out(filename, std::ios::binary);
...
string generate_bitstream(std::vector <std::vector<Sequence>>& AC, float* dc_coef ){

    string bitstream;
    int b = 0;
    for (auto& block : AC){
        int dc_val = static_cast<int>(dc_coef[b]);
        b ++;
        bitstream += glob(dc_val);
        for (auto& seq : block){
            int zeros = seq.nbre_zero;
            for (auto v : seq.val){
                int coeff = static_cast<int>(v);
                bitstream += glob(zeros);
                bitstream += glob(coeff);
                zeros = 0;
            }
...
size_t bitstream_size_bits(const string& filename)
{
    ...
    size_t size_bytes = static_cast<size_t>(file.tellg());
    size_t bits = size_bytes*8;
    cout << "Bits number for encoding is : " << bits << endl;
    return bits;  // return in bits !
}

float total_symbol(std::vector <std::vector<Sequence>>& AC){

    int symb_count = 0;
    int symb_tot = 0;
    int block_count = 0;
    float average_symb = 0.0f;

    for (auto& block : AC){
        for (auto& seq : block){
            symb_tot += seq.val.size();
        }
        block_count ++;
    }
    average_symb = symb_tot/block_count;

    cout << "Total symbol with RLE encoding is : "<<symb_tot<<endl;

    cout << "Average symbols per 8x8 block is : " << average_symb << endl;

    return symb_tot;
}

void generate_bistream_txt(std::vector <std::vector<Sequence>>& AC, float* dc_coef, const string& filename){
    std::ofstream out(filename);
    int b = 0;
    for (auto& s : AC) {
        string dc = glob(dc_coef[b]);
        b++;
        out << dc;
        for (auto& seq : s) {
            int run = seq.nbre_zero;
            for (auto v : seq.val) {
                int val = static_cast<int>(v);
                string code_run = glob(run);
                string code_val = glob(val);
                out << code_run << code_val;
                run = 0;
            }
        }
    }
...
int main(){
    ...
    /*float dc_test [5] = {10, 5, 2 , 8 , 6};
    float* delta_test = delta_dc(dc_test,5);
    cout<< "delta number is : "<<endl;
    for (size_t i = 0; i<5 ; i++){
        cout<<delta_test[i];
    }
    cout <<endl;
    float* reconstr_test = reconstruct_from_delta(delta_test,5);
    cout<< "reconstruct number is : "<<endl;
    for (size_t i = 0; i<5 ; i++){
        cout<<reconstr_test[i];
    }
    cout <<endl;*/


    // ************************ SESSION 6 *************************
    /*std::vector<std::vector<Sequence>> AC = {
        { {2, {5, 2}}, {1, {-1}} },
        { {0, {7}} }
    };*/
    std::vector <Occurence> histogram = occurence_calc(Encoded_RLE_data);
    std::vector<float> occurence_hist = norm_occurence(histogram);
    /*for (auto s : histogram) {
        std::cout << "(" << s.nbre_zero << "," << s.val << ") -> " << s.counter << "\n";
    }*/
    cout << "Histogram size is : "<< size(occurence_hist) <<endl;
    Raw_save("Occurence_RLE_hist.raw",occurence_hist.data(),size(occurence_hist));

    float H = entropy_calc(histogram);

    generate_bistream_txt(Encoded_RLE_data,dc_coef,"bitstream.txt");
    string bitstream_AC = generate_bitstream(Encoded_RLE_data, dc_coef);
    write_binary_file(bitstream_AC,"bitstream.bin");
    size_t bits_encoding = bitstream_size_bits("bitstream.bin");

    size_t true_bits_nbre = bitstream_AC.size();
    double bits_per_pixel = (true_bits_nbre)/double(height*width);
    cout << "Bits per pixel with current encoding RLE + glob is : " << bits_per_pixel << endl;
    float compression = 8/bits_per_pixel;
    cout << "Compression rate is : " << compression << endl;
    float tot_symb_AC = total_symbol(Encoded_RLE_data);
    double bits_per_symb = bits_encoding/tot_symb_AC;
    cout << "Bits per symbol with current encoding is : "<<bits_per_symb<<endl;
